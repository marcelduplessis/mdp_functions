...
if __name__ == '__main__':
    ...
    path = '/Users/xduplm/Google Drive/My Drive/teaching/2023/oc4920/assignments/assignment_2/COARE3.6/'
    fil = 'ooi_data.txt' 
    ...
    data = pd.read_csv(path+fil, skiprows=0)[['jd', 'u', 'zu', 't', 'zt', 'rh', 'zq', 'P', 'ts', 'sw_dn', 'lw_dn', 'lat', 'lon', 'zi', 'rain', 'zts', 'Ss', 'cp', 'sigH']]
    
    jd    = data.iloc[:,0].values
    u     = data.iloc[:,1].values
    zu    = data.iloc[:,2].values
    tair  = data.iloc[:,3].values
    zt    = data.iloc[:,4].values
    rh    = data.iloc[:,5].values
    zq    = data.iloc[:,6].values
    P     = data.iloc[:,7].values
    tsea  = data.iloc[:,8].values
    sw_dn = data.iloc[:,9].values
    lw_dn = data.iloc[:,10].values
    lat   = data.iloc[:,11].values
    lon   = data.iloc[:,12].values
    zi    = data.iloc[:,13].values
    rain  = data.iloc[:,14].values
    zts   = data.iloc[:,15].values
    sss   = data.iloc[:,16].values
    cp    = data.iloc[:,17].values
    sigH  = data.iloc[:,18].values    
    zrf_u = 10.0;
    zrf_t = 10.0;
    zrf_q = 10.0;
    
    # This is what the function is asking for:
    # coare36vnWarm_et(Jd, U, Zu, Tair, Zt, RH, Zq, P, Tsea, SW_dn, LW_dn, Lat, Lon, Zi, Rainrate, Ts_depth, Ss, cp=None, sigH=None,zrf_u = 10.0,zrf_t = 10.0,zrf_q = 10.0): 
    # coare36vnWarm_et(jd, u, zu, tair, zt, rh, zq, P, tsea, sw_dn, lw_dn, lat, lon, zi, rain, zts, sss, cp=None, sigH=None, zrf_u = 10.0, zrf_t = 10.0, zrf_q = 10.0):
    
     # 'jd', # year day or julian day, where day Jan 1 00:00 UTC = 0
     # 'u', # water-relative wind speed magnitude (m/s) at height zu (m)
     # 'zu', # heights of the observations (m)
     # 't', # air temperature (degC) at height zt (m)
     # 'zt', # heights of the observations (m)
     # 'rh', # relative humidity (#) at height zq (m)
     # 'zq', # heights of the observations (m)
     ...
     # 'zts' depth (m) of water temperature measurement, positive for below surface 
     # 'sss', # sea surface salinity (PSU)
     # 'cp', # phase speed of dominant waves (m/s) computed from peak period
     # 'sigH', # significant wave height (m),
  
    A=coare36vnWarm_et(jd, u, zu, tair, zt, rh, zq, P, tsea, sw_dn, lw_dn, lat, lon, zi, rain, zts, sss, None, None, zrf_u, zrf_t, zrf_q)
    
    from datetime import datetime
    date = datetime.now()
    fnameA = os.path.join(path,f'ooi_oc4920_coare_output_{date.strftime("%Y%m%d%H")}_withnowavesinput_withwarmlayer.txt')
    ...
    np.savetxt(fnameA,A,fmt='%.18e',delimiter='\t',header=A_hdr, comments='')
